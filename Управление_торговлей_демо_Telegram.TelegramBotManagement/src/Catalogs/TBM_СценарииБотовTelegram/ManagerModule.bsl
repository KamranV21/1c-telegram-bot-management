...
Процедура ИнициализироватьСценарийАвторизации() Экспорт
	...
	ЭтапПроверки.ПроизвольныйКод = "Параметры.КнопкиКлавиатуры.Очистить();
								   ...
								   |
								   |Результат.Ответ = ""Введите одноразовый пароль (OTP)"";
								   |
								   |ЗавершитьЭтап(Параметры.НаименованиеТекущегоЭтапа);
								   |
	...
	ЭтапВводаOTP.ПроизвольныйКод = "ДанныеПароля = РегистрыСведений.TBM_ОдноразовыеПаролиАвторизацииЧатовTelegram.ОдноразовыйПарольАвторизацииЧата(ЗапрошенныйСценарий);
								   |
								   ...
								   |Иначе
								   |
								   |Результат.Ответ = ""Неверный пароль"";
								   |
								   |КонецЕсли;";
