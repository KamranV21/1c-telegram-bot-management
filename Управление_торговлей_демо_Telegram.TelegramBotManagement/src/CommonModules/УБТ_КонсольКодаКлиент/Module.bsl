...
#Если Не ВебКлиент Тогда
	...
	Исключение
		ВывестиОшибку(Форма, "Не удалось извлечь исходники" + Символы.ПС + ОписаниеОшибки(), Истина);
	КонецПопытки;
...
Процедура ПослеПроверкиСуществованияКаталога(Существует, ДопПараметры) Экспорт
	...
	Если Существует Тогда
		...
		ФайлНаДиске.НачатьПроверкуСуществования(Оповещение);
	Иначе
		ВывестиОшибку(Форма, "Не удалось создать каталог для исходников", Истина);
	КонецЕсли;
...
Процедура УстановитьТекст(Форма, Текст, Позиция, УчитыватьОтступПервойСтроки)
...
Процедура ПриЗакрытииКонструктораЗапросов(Текст, ДопПараметры) Экспорт
	...
	Если Текст <> Неопределено Тогда
		...
		УстановитьТекст(Форма, Текст, Неопределено, Истина);
...
Процедура ВызватьКонструкторЗапроса(Форма, ПараметрыЗапроса)
...
Процедура ПриЗакрытииКонструктораФорматнойСтроки(ФорматнаяСтрока, ДопПараметры) Экспорт
	...
	Если ФорматнаяСтрока <> Неопределено Тогда
		...
		УстановитьТекст(Форма, ФорматнаяСтрока, ДопПараметры, Ложь);
	КонецЕсли;
