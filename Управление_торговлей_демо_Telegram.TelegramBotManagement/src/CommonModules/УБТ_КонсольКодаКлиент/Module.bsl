...
// Вызывается при нажатии на кнопку "Быстрые подсказки".
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма
Процедура БыстрыеПодсказки(Форма) Экспорт

	Форма.БыстрыеПодсказки = Не Форма.БыстрыеПодсказки;
	View(Форма).enableQuickSuggestions(Форма.БыстрыеПодсказки);
	УправлениеБыстрымиПодсказками(Форма);

КонецПроцедуры

// Вызывается при нажатии на кнопку "Карта кода".
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма
Процедура КартаКода(Форма) Экспорт

	Форма.ВидимостьКартыКода = Не Форма.ВидимостьКартыКода;
	View(Форма).minimap(Форма.ВидимостьКартыКода);
	УправлениеВидимостьюКартыКода(Форма);
...
Процедура ПослеУдаленияВременныхФайлов(ДопПараметры) Экспорт
	...
	ДанныеМакета = ПолучитьИзВременногоХранилища(Форма.АдресМакета);
...
Процедура УправлениеВидимостьюКартыКода(Форма)

	Форма.Элементы.ПроизвольныйКодКартаКода.Пометка = Форма.ВидимостьКартыКода;
