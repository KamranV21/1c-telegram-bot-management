...
Процедура ОтветитьНаПолученныеСообщения(Бот, ТаблицаСообщений)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
						  ...
						  |	ЕСТЬNULL(ПерсональныеКоманды.Действие, ЕСТЬNULL(ОбщиеКоманды.Действие, ЗНАЧЕНИЕ(Справочник.TBM_ДействияБотовTelegram.ПустаяСсылка))) КАК Действие
						  ...
						  |	ЕСТЬNULL(TBM_ЧатыTelegram.Ссылка, ЗНАЧЕНИЕ(Справочник.TBM_ЧатыTelegram.ПустаяСсылка)) КАК Чат,
						  |	ВтДействия.Действие КАК Действие,
						  |	ВтДействия.Действие.ТипДействия КАК ТипДействия
						  |ИЗ
						  |	ВтДействия КАК ВтДействия
						  |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.TBM_ЧатыTelegram КАК TBM_ЧатыTelegram
						  |		ПО ВтДействия.ИдентификаторЧата = TBM_ЧатыTelegram.Код
						  |ИТОГИ ПО
						  |	Действие");
	Запрос.УстановитьПараметр("ТаблицаСообщений", ТаблицаСообщений);
	Запрос.УстановитьПараметр("Бот", Бот);

	ВыборкаДействия = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
...
Процедура ВыполнитьДействиеИОтправитьСообщениеВЧат(Бот, ВыборкаДействия)
	...
	ПроизвольныйКод = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Действие, "ПроизвольныйКод");
...
Процедура ВыполнитьПроизвольныйКод(ПроизвольныйКод, Параметры, Результат)
