...
// Вызывается при создании формы, в которой размещена консоль кода.
// 
// Параметры:
...
Процедура ПриСозданииНаСервере(Форма) Экспорт
	...
	Форма.АдресМакета = ПоместитьВоВременноеХранилище(ИсходникиКонсолиКода, Новый УникальныйИдентификатор);

	КоллекцияОбщихМодулей = ПолучитьКоллекциюОбщихМодулей();
	Форма.АдресОбщихМодулей = СохранитьОбщиеМодулиВХранилище(КоллекцияОбщихМодулей);

	Форма.БыстрыеПодсказки = Истина;
	Форма.ВидимостьКартыКода = Истина;
...
//  АдресОбновления - Строка - Адрес обновления
// 
// Возвращаемое значение:
//  Строка - Получить описание объекта метаданных
	...
	Если ЗначениеЗаполнено(ИмяКоллекции) Тогда
		...
		Исключение
			ВызватьИсключение (НСтр(
				"az = 'Mətadatlar kolleksiyasını saxlamaq mümkün olmadı:';en = 'Failed to save the metadata collection:';ru = 'Не удалось сохранить коллекцию метаданных:'")
...
// 
// Параметры:
//  ТипОбъектов - Строка - Тип объектов
//  АдресОбновления - Строка - Адрес обновления
// 
// Возвращаемое значение:
//  Строка - Получить список объектов метаданных
...
#Область ПолучениеЗначенияПеременныхДляВыводаВТабло
...
// 
// Возвращаемое значение:
//  Строка - Получить описание переменной
Функция ПолучитьОписаниеПеременной(ИдентификаторПеременной, ИмяПеременной, ПутьКДанным, АдресХраненияПеременных) Экспорт
	...
	Попытка
		...
		Иначе
			ВызватьИсключение (НСтр(
				"az = 'Naməlum dəyişən tipi';en = 'Unknown variable type';ru = 'Неизвестный тип переменной'"));
		КонецЕсли;
...
// Параметры:
//  Текст - Строка - Текст
//  ПоказыватьЗначенияПеременныхПослеВыполнения - Булево - Показывать значения переменных после выполнения
// 
// Возвращаемое значение:
//  Структура - Выполнить код на сервере:
// * Успешно - Булево
// * ИсходнаяСтрока - Строка
// * ОписаниеОшибки - Строка
// * ОписаниеПеременных - Строка
// * АдресХраненияПеременных - Строка
Функция ВыполнитьКодНаСервере(Текст, ПоказыватьЗначенияПеременныхПослеВыполнения) Экспорт
...
Функция ПолучитьПредставлениеТаблицыЗначений(Значение, Заголовок, Смещение, КешСсылок)
	...
	Для Каждого СтрокаТЗ Из Значение Цикл
		...
		МассивСтрок.Добавить(Маркер + СтрШаблон("%1: ", НСтр("az = 'Sətr';en = 'Row';ru = 'Строка'")) + Индекс);
...
Функция ПолучитьПредставлениеДереваЗначений(Дерево, Смещение, КешСсылок)
	...
	МассивСтрок = Новый Массив;
	МассивСтрок.Добавить(СтрШаблон("**%1 (%2):**", НСтр("az = 'Dəyər ağacı';en = 'Value tree';ru = 'Дерево значений'"),
		Дерево.Строки.Количество()));
...
Функция ПолучитьПредставлениеСтрокиТаблицыЗначений(Значение, Смещение, КешСсылок)
	...
	Представление = СтрШаблон("**%1**", НСтр(
		"az = 'Dəyər cədvəli sətiri';en = 'Value table row';ru = 'Строка таблицы значений:'")) + Символы.ПС;
...
Функция ПолучитьПредставлениеЭлементаСпискаЗначений(ЭлементСписка, Смещение, КешСсылок)
	...
	МассивСтрок = Новый Массив;
	МассивСтрок.Добавить(СтрШаблон("**%1**:", НСтр(
		"az = 'Dəyər siyahısı elementi';en = ' Value list item';ru = 'Элемент списка значений'")));

	Маркер = ДополнитьСтроку("- ", Смещение * 4, " ");
	МассивСтрок.Добавить(Маркер + СтрШаблон("%1: ", НСтр("az = 'Dəyər';en = 'Value';ru = 'Значение'"))
		+ ПолучитьПредставлениеПеременной(ЭлементСписка.Значение, Смещение, КешСсылок));
	...
	МассивСтрок.Добавить(Маркер + СтрШаблон("%1: ", НСтр("az = 'Təsvir';en = 'Representation';ru = 'Представление'"))
		+ Представление);
	МассивСтрок.Добавить(Маркер + СтрШаблон("%1: ", НСтр("az = 'Nişan';en = 'Mark';ru = 'Пометка'"))
...
Функция ПолучитьПредставлениеКлючаЗначения(КлючЗначение, Смещение, КешСсылок)
	...
	МассивСтрок = Новый Массив;
	МассивСтрок.Добавить(СтрШаблон("**%1**:", НСтр(
		"az = 'Açar və dəyər';en = ' Key and value';ru = 'Ключ и значение'")));

	Маркер = ДополнитьСтроку("- ", Смещение * 4, " ");
	МассивСтрок.Добавить(Маркер + СтрШаблон("%1: ", НСтр("az = 'Açar';en = 'Key';ru = 'Ключ'")) + КлючЗначение.Ключ);
	МассивСтрок.Добавить(Маркер + СтрШаблон("%1: ", НСтр("az = 'Dəyər';en = 'Value';ru = 'Значение'"))
		+ ПолучитьПредставлениеПеременной(КлючЗначение.Значение, Смещение, КешСсылок));
...
Функция ПолучитьПредставлениеЗапроса(Запрос, Смещение, КешСсылок)
	...
	МассивСтрок = Новый Массив;
	МассивСтрок.Добавить(НСтр("az = 'Sorğu';en = ' Query';ru = 'Запрос'"));

	Маркер = ДополнитьСтроку("- ", Смещение * 4, " ");

	Если Запрос.МенеджерВременныхТаблиц = Неопределено Тогда
		МассивСтрок.Добавить(Маркер + СтрШаблон("%1: ", НСтр(
			"az = 'Müvəqqəti Cədvəllərin Meneceri: Təyin edilməyib';en = 'Manager of Temporary Tables: Not set';ru = 'МенеджерВременныхТаблиц: Не установлен'")));
	Иначе
		МассивСтрок.Добавить(Маркер + СтрШаблон("%1: ", НСтр(
			"az = 'Müvəqqəti Cədvəllərin Meneceri: Təyin edilib';en = 'Manager of Temporary Tables: Set';ru = 'МенеджерВременныхТаблиц: Установлен'")));
	КонецЕсли;
	...
	МассивСтрок.Добавить(Маркер + СтрШаблон("%1: ", НСтр("az = 'Mətn';en = 'Text';ru = 'Текст'")) + Символы.ПС
		+ МаркерТекста + Запрос.Текст);
...
Функция ПолучитьОписаниеДереваЗначений(ДеревоЗначений, ИмяПеременной, ПутьКДанным, КешСсылок)
	...
	Для Каждого СтрокаДерева Из ДеревоЗначений.Строки Цикл
		Имя = СтрШаблон("%1: ", НСтр("az = 'Sətr';en = 'Row';ru = 'Строка'")) + Индекс;
