...
// Удаляет все персоанльные команды чата.
// 
// Параметры:
//  Бот - СправочникСсылка.УБТ_БотыTelegram - Бот
//  Чат - СправочникСсылка.УБТ_ЧатыTelegram - Чат
...
Процедура ВыполнитьЭтапОбщения(Общение, ПараметрыВыполненияЭтапа)
	...
	Попытка
		...
		Для Каждого ДанныеФайла Из Результат.Файлы Цикл
			ДвоичныеДанныеСообщения = ТелоВыгрузкиДокумента(ИдентификаторЧата, ДанныеФайла.ДвоичныеДанные,
				ДанныеФайла.НаименованиеСРасширением);
			ОтправитьДокументВЧат(Бот, ДвоичныеДанныеСообщения);
		КонецЦикла;
...
Функция ТелоВыгрузкиДокумента(ИдентификаторЧата, ДвоичныеДанные, ПолноеИмяФайла)
	...
	ЗаписьДанных.ЗаписатьСтроку("Content-Disposition: form-data; name=""document""; filename=""" + ПолноеИмяФайла
		+ """");
