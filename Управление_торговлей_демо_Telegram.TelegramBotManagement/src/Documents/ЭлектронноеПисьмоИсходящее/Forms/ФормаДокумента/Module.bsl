...
Процедура УБТ_НачатьПередачуСообщенияПоTelegram()
	...
	Результат = УБТ_ОтправитьСообщениеПоTelegramНаСервере();
...
Функция УБТ_ОтправитьСообщениеПоTelegramНаСервере()
	...
	ИдентификаторыЧатов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Чаты, "Код");

	Результат = Истина;

	ДанныеФайлов = Новый Массив;
	Для Каждого СтрокаВложения Из Вложения Цикл
		ДанныеФайла = УБТ_РаботаСБотамиTelegram.НоваяСтруктураФайлаСообщенияОтвета();
		ДанныеФайла.ДвоичныеДанные = РаботаСФайлами.ДвоичныеДанныеФайла(СтрокаВложения.Ссылка);
		ДанныеФайла.НаименованиеСРасширением = СтрокаВложения.ИмяФайла;
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.УБТ_ЧатыПолучатели Цикл
		...
		Если Объект.ЕстьВложения Тогда
			...
			Для Каждого ДанныеФайла Из ДанныеФайлов Цикл
				...
				Сообщение = УБТ_РаботаСБотамиTelegram.НоваяСтруктураСообщенияСДокументом();
				...
				Если Объект.ТипТекста = Перечисления.ТипыТекстовЭлектронныхПисем.ПростойТекст Тогда
					Сообщение.caption = Объект.Текст;
				Иначе
					Сообщение.caption = Объект.ТекстHTML;
					Сообщение.parse_mode = "HTML";
				КонецЕсли;

				ТелоВыгрузкиДокумента = УБТ_РаботаСБотамиTelegram.ТелоВыгрузкиДокумента(Сообщение);

				РезультатОтправкиСообщения = УБТ_РаботаСБотамиTelegram.ОтправитьДокументВЧат(Объект.УБТ_БотTelegram,
					ТелоВыгрузкиДокумента);
		...
		Иначе
			...
			РезультатОтправкиСообщения = УБТ_РаботаСБотамиTelegram.ОтправитьСообщениеВЧат(Объект.УБТ_БотTelegram,
				Сообщение);

		КонецЕсли;

		Если Не РезультатОтправкиСообщения Тогда
			Результат = Ложь;
			Прервать;
		КонецЕсли;
