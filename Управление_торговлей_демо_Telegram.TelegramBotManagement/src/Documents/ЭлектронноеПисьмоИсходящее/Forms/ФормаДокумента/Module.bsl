...
Функция УБТ_ОтправитьСообщенияВЧатыTelegram()
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.УБТ_ЧатыПолучатели Цикл
		...
		РезультатПередачиСообщения = УБТ_РаботаСБотамиTelegram.ОтправитьСообщениеВЧат(Объект.УБТ_БотTelegram, Сообщение);

		СтрокаТабличнойЧасти.Доставлено = РезультатПередачиСообщения <> Неопределено;
