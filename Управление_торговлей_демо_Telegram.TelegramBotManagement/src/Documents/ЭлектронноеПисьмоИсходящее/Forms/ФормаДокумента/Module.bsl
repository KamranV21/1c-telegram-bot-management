...
Функция УБТ_ОтправитьГруппуМедиафайловВЧатыTelegram()
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.УБТ_ЧатыПолучатели Цикл
		...
		РезультатОтправкиСообщения = СообщениеОтвета <> Неопределено;

		Если РезультатОтправкиСообщения И СтрокаТабличнойЧасти.НомерСтроки = 1 Тогда
...
Функция УБТ_ОтправитьСообщенияВЧатыTelegram()
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.УБТ_ЧатыПолучатели Цикл
		...
		РезультатОтправкиСообщения = УБТ_РаботаСБотамиTelegram.ОтправитьСообщениеВЧат(Объект.УБТ_БотTelegram, Сообщение);
